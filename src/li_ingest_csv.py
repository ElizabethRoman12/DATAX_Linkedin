...
def insert_extra(entidad_id, tipo_entidad, plataforma, fecha, row, columnas_principales=[], publicacion_id=None):
    ...
    # 🔹 Excluir solo las métricas principales confirmadas
    METRICAS_EXCLUIR = {
        "Impresiones", "Clics",
        "Impresiones (totales)", "Clics (totales)",
        "Reacciones (total)", "Comentarios (totales)",
        "Veces compartido (total)", "Tasa de interacción (total)",
        "Visitantes únicos en total (total)", "Visualizaciones de la página en total (total)"
    }

    for col, val in row.items():
        if col in columnas_principales or col in METRICAS_EXCLUIR or val is None or str(val).strip() == "":
            continue
...
def ingest_indicadores(path=f"{CLEAN_DIR}/Contenido_Indicadores.csv"):
    ...
    for _, row in df.iterrows():
        ...
        insert_extra(PAGE_ID, "indicadores", "linkedin", fecha, row, [
            "Fecha",
            "Impresiones (totales)", "Clics (totales)", "Reacciones (total)",
            "Comentarios (totales)", "Veces compartido (total)", "Tasa de interacción (total)"
        ])
